...
| 4 | ตั้งค่าธีม+ภาษา บันทึกลง DB ได้ แต่ไม่มีผลกับแอป | 🟠 หลอกตา | ❌ **รอผู้ใช้ตัดสินใจ** |
| 5 | เก็บกวาดเล็กน้อย (logout ปลายทาง, `Pressable` ผิดกฎ #4) | 🟡 เล็ก | ✅ 2026-08-13 |

**เหลือก้อนเดียว: ก้อน 4** (ต้องเลือกทางก่อนถึงจะสั่ง Codex ได้ — ดูหัวข้อก้อน 4)
...
## 🟠 ก้อนที่ 4 — ตั้งค่าธีม/ภาษา บันทึกได้ แต่ไม่มีผลกับแอป ← **ก้อนเดียวที่เหลือ**

`src/app/settings.tsx:36-46` ยิง `usersApi.updateSettings()` ขึ้น server จริง **แต่**
- **ธีม** — ทั้งแอปใช้ palette `Brand` ค่าคงที่ตัวเดียว ไม่มี dark mode จริง
  (`src/hooks/use-theme.ts` มีอยู่ แต่ถูกใช้แค่ใน `themed-text/themed-view/collapsible`
  ซึ่งเป็นไฟล์ template ที่หน้าจอจริงไม่ได้ใช้เลย)
- **ภาษา** — ไม่มีระบบ i18n ข้อความไทยฝังตายในโค้ดทุกไฟล์
- **notifyPromo** — ไม่มีระบบ push notification

**ผู้ใช้เห็นสวิตช์ กดแล้วติ๊กเปลี่ยน เชื่อว่าเปลี่ยนแล้ว แต่ไม่มีอะไรเกิดขึ้น**

**ตัดสินใจก่อนสั่ง Codex — งานนี้ใหญ่กว่าที่เห็น เลือก 1 ใน 3:**
1. **ถูกที่สุด (แนะนำ):** ใส่ป้าย "เร็วๆ นี้" + disable ทั้ง 3 อย่าง ~30 นาที
2. **กลาง:** ทำ dark mode จริง — ต้องแปลง `Brand` เป็น 2 ชุดสี + context + แก้ทุกหน้า ~1-2 วัน
3. **แพง:** ทำ i18n ไทย/อังกฤษ — ต้องถอดข้อความทุกไฟล์ออกเป็น dictionary ~2-3 วัน

> ⚠️ ยังไม่ต้องสั่ง Codex จนกว่าจะเลือกได้ — ถ้าเลือกข้อ 1 บอก Claude ให้เขียน prompt ให้
